...
public class AxolotlAi {
   ...
   private static void initFightActivity(Brain<Axolotl> var0) {
      var0.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 0, ImmutableList.of(new StopAttackingIfTargetInvalid(), new SetWalkTargetFromAttackTargetIfTargetOutOfReach(AxolotlAi::getSpeedModifierChasing), new MeleeAttack(20), new EraseMemoryIf(AxolotlAi::isBreeding, MemoryModuleType.ATTACK_TARGET)), MemoryModuleType.ATTACK_TARGET);
   }

   private static void initCoreActivity(Brain<Axolotl> var0) {
      var0.addActivity(Activity.CORE, 0, ImmutableList.of(new LookAtTargetSink(45, 90), new MoveToTargetSink(), new ValidatePlayDead(), new CountDownTemptationTicks()));
   }

   private static void initIdleActivity(Brain<Axolotl> var0) {
      var0.addActivity(Activity.IDLE, ImmutableList.of(Pair.of(0, new RunSometimes(new SetEntityLookTarget(EntityType.PLAYER, 6.0F), IntRange.of(30, 60))), Pair.of(1, new RunOne(ImmutableList.of(Pair.of(new AnimalMakeLove(EntityType.AXOLOTL, 0.2F), 1), Pair.of(new FollowTemptation(AxolotlAi::getSpeedModifier), 1), Pair.of(new BabyFollowAdult(ADULT_FOLLOW_RANGE, AxolotlAi::getSpeedModifierFollowingAdult), 1)))), Pair.of(2, new StartAttacking(AxolotlAi::findNearestValidAttackTarget)), Pair.of(2, new TryFindWater(6, 0.15F)), Pair.of(3, new GateBehavior(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableSet.of(), GateBehavior.OrderPolicy.ORDERED, GateBehavior.RunningPolicy.TRY_ALL, ImmutableList.of(Pair.of(new RandomSwim(0.5F), 2), Pair.of(new RandomStroll(0.15F), 2), Pair.of(new SetWalkTargetFromLookTarget(AxolotlAi::getSpeedModifier, 3), 3), Pair.of(new RunIf(Entity::isInWaterOrBubble, new DoNothing(30, 60)), 5), Pair.of(new RunIf(Entity::isOnGround, new DoNothing(200, 400)), 5))))));
   }
   ...
   private static Optional<? extends LivingEntity> findNearestValidAttackTarget(Axolotl var0) {
      return isBreeding(var0) ? Optional.empty() : var0.getBrain().getMemory(MemoryModuleType.NEAREST_HOSTILE);
   }
